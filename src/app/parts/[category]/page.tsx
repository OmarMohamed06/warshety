...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return generateSeoMeta({
    title: `${name} — Browse Subcategories`,
    description: `Shop all ${name} categories in Egypt. Compatible parts with fast delivery and verified vendors. قطع ${name}.`,
    path: `/parts/${category}`,
  });
}
    ...
    <div className="min-h-screen bg-[#f6f6f8] dark:bg-[#111621]">
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-6 py-10">
          ...
          <h1 className="text-4xl font-black mb-1">{data.name}</h1>
          {data.nameAr && (
            <p className="text-slate-400 text-lg mb-2">{data.nameAr}</p>
          )}
          <p className="text-slate-500">
            Select a subcategory to find compatible parts for your vehicle.
          </p>
        </div>
      </div>
            ...
            {data.subs.map((sub) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-400">{sub.nameAr}</p>
